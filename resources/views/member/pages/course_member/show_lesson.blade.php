...
@section('content')
        ...
        <div class="container page__container">
            <nav class="course-nav">
                @foreach ($list_lesson as $k => $v)
                    ...
                    <a data-toggle="tooltip" data-placement="bottom" data-title="{{ $v->title }}"
                        href="{{ $data['croute'] . 'show_lesson/' . enc($v->id) }}"><span
                            class="material-icons">{{ $v->lesson_icon_learning(info_user_id()) }}</span>
                    </a>
                @endforeach
            </nav>
            ...
            <div class="d-flex flex-column flex-sm-row align-items-center justify-content-start">
                @if (array_key_exists('previous_page', $list_route))
                    <a href="{{ $list_route['previous_page'] }}" class="btn btn-outline-white mb-16pt mb-sm-0 mr-sm-16pt"><i
                            class="material-icons icon--left">arrow_left</i>Sebelumnya</a>
                @endif

                @if (!$results_data->is_done_by_user(info_user_id()))
                    <a id="btn_finish_lesson" name="btn_finish_lesson"
                        class="btn btn-outline-white mb-16pt mb-sm-0 mr-sm-16pt"><i
                            class="material-icons icon--left">check_circle</i>Selesai</a>
                @endif

                @if (array_key_exists('next_page', $list_route))
                    <a href="{{ $list_route['next_page'] }}"
                        class="btn btn-outline-white mb-16pt mb-sm-0 mr-sm-16pt">Selanjutnya <i
                            class="material-icons icon--right">arrow_right</i></a>
                @endif
    ...
    @if ($results_data['type'] == App\Models\Lesson::TYPE_VIDEO)
        ...
        <div class="page-section bg-white">
            <div class="container page__container">
                <div class="d-flex align-items-center mb-heading">
                    ...
                    <a href="{{ $list_route['create_discussion'] }}" class="text-underline ml-auto">Berikan Pertanyaan</a>
                </div>
    ...
    @else
        @include('member.pages.course.components.show_title_quiz_nav')
        @livewire('member.online-course.quiz', [
            'lesson_id' => enc($results_data['id']),
        ])
    @endif
@stop
